...
echo -e "${CLR_BLD_GRN}Building AOSP for $DEVICE${CLR_RST}"
...
BUILD="aosp"
BUILD_DATE="$(/bin/date -u +%Y%m%d-%H%M)"
checkExit
echo -e ""
...
# Build signed rom package if specified
    ...
    sign_target_files_apks -o -d $KEY_MAPPINGS \
        ...
        $BUILD-signed-target_files-$BUILD_DATE.zip
    ...
    if [ -z "${DELTA_TARGET_FILES}" ]; then
        ...
        ota_from_target_files -k $KEY_MAPPINGS/releasekey \
            $BUILD-signed-target_files-$BUILD_DATE.zip \
            $BUILD-signed-fullota-$BUILD_DATE.zip

        checkExit

        if [ "$FLAG_PUSH_OTA" = 'y' ]; then
            echo -e "${CLR_BLD_BLU}Executing push_ota.sh${CLR_RST}"
            ./push_ota.sh $BUILD-signed-fullota-$BUILD_DATE.zip $BUILD-signed-target_files-$BUILD_DATE.zip
            checkExit
        fi
    fi
    ...
    if [ "$DELTA_TARGET_FILES" ]; then
        ...
        ota_from_target_files -k $KEY_MAPPINGS/releasekey \
            ...
            $BUILD-signed-target_files-$BUILD_DATE.zip \
            $BUILD-delta-$BUILD_DATE.zip
        checkExit

        if [ "$FLAG_PUSH_OTA" = 'y' ]; then
            echo -e "${CLR_BLD_BLU}Executing push_ota.sh${CLR_RST}"
            ./push_ota.sh $BUILD-delta-$BUILD_DATE.zip $BUILD-signed-target_files-$BUILD_DATE.zip
            checkExit
        fi
    fi
    ...
    if [ "$FLAG_IMG_ZIP" = 'y' ]; then
        ...
        img_from_target_files \
            $BUILD-signed-target_files-$BUILD_DATE.zip \
            $BUILD-signed-image-$BUILD_DATE.zip
        checkExit
    fi
# Build rom package
    ...
    ota_from_target_files \
        "$OUT"/obj/PACKAGING/target_files_intermediates/aosp_$DEVICE-target_files.zip \
        $BUILD-fullota-$BUILD_DATE.zip
